...
(defn- exported-row
  "Capture the JSONEachRow body the exporter writes for one span.

  A non-durable trace insert hands chDB the rows as data rather than as SQL, so
  the body arrives through `insert-rows!`; the durable path still serializes a
  whole statement. Both are redefined so this assertion tests the projected
  columns rather than the insert mechanism."
  [descriptor-set target attributes]
  (let [payload (atom nil)
    ...
    (with-redefs [jdbc/execute!
                  (fn [_ sql]
                    (reset! payload (second (str/split sql #"FORMAT JSONEachRow\n" 2)))
                    {:count 1})
                  chdb/insert-rows!
                  (fn [_ _ _ body] (reset! payload body) {:count 1})]
