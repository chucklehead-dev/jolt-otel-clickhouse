(ns otel.exporter.scalar-abba
  (:require [clojure.data.json :as json] [clojure.java.io :as io]
            ...
            [otel.exporter.untyped-encoder :as prototype]
...
(defn run-writer! [root]
  ...
  (let [fixture-text (slurp (System/getenv "BENCH_FROZEN_FIXTURE"))
        ...
        _ (check! (= (f/sha256 (slurp (io/resource "otel/exporter/untyped_encoder.clj")))
                     (System/getenv "BENCH_PROTOTYPE_SOURCE_SHA256")) "loaded prototype hash")
        encoder (when (= "B" arm) (prototype/compile-plan))
        _ (when (= "B" arm)
            (check! (ifn? encoder) "prototype plan active")
            (check! (every? prototype/eligible? spans) "fixture uses generated path"))
        rows (mapv #(#'exporter/span-row % nil) spans)
        payload (#'exporter/json-each-row-payload rows)
        _ (when encoder (check! (= payload (prototype/payload encoder spans)) "prototype exact payload"))
    ...
    (with-open [connection (jdbc/connection (durable/writer-dbspec options))]
      ...
      (let [writer (exporter/exporter {:connection connection :durable? true :create-schema? false :signals #{:spans}})
            op #(if encoder (prototype/export-spans! writer spans encoder)
                          (export/export-spans! writer spans))
